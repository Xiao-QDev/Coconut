...
impl<'ctx> CodeGenerator<'ctx> {
    ...
    fn declare_function(&self, func: &Function) -> Result<(), String> {
        ...
        if func.return_type == Some(Type::Void) || func.return_type.is_none() {
            ...
            match return_ty {
                inkwell::types::BasicTypeEnum::IntType(t) => {
                    let fn_type = t.fn_type(&param_types, false);
                    self.module().add_function(&func.name, fn_type, None);
                }
    ...
    fn generate_function(&self, func: &Function) -> Result<(), String> {
        ...
        for (i, param) in function.get_params().iter().enumerate() {
            if i < func.params.len() {
                ...
                self.set_var_type(&func.params[i].name, func.params[i].param_type.clone());
                let param_type = self.type_to_llvm_basic(&func.params[i].param_type)?;
                let ptr = self
                    .builder()
                    .build_alloca(param_type, &func.params[i].name)
                    .unwrap();
    ...
    fn default_value_for_type(&self, ty: &Type) -> Result<BasicValueEnum<'ctx>, String> {
        match ty {
            ...
            Type::Array(_, _) | Type::Struct(_) => {
                Err("Array/Struct default value not yet supported".to_string())
            }
            ...
            Statement::VarDecl(var_decl) => {
                ...
                if matches!(var_type, Type::Array(_, _) | Type::Struct(_)) {
                    ...
                    let ptr = self
                        ...
                        .unwrap();
                    let zero_init = llvm_type.const_zero();
                    self.builder().build_store(ptr, zero_init).unwrap();
                    self.named_values
                        .borrow_mut()
