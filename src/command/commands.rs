...
fn lex_and_parse(source: &str, filename: &str) -> Result<Program, String> {
    let mut lexer = Lexer::new(source);
    let tokens = lexer
        .tokenize()
        .map_err(|e| format!("{}: {}", filename, e))?;
    let mut parser = Parser::new(tokens);
    parser.parse().map_err(|e| format!("{}: {}", filename, e))
}
...
pub fn compile_source(filename: &str) -> Result<PathBuf, String> {
    ...
    let program = lex_and_parse(&source, filename)?;
...
pub fn compile_all() -> Result<PathBuf, String> {
    ...
    let mut main_program =
        lex_and_parse(&main_source, "src/main.cocl").map_err(|e| format!("{}", e))?;
    ...
    for import in &main_program.imports {
        ...
        for import_file in &import_files {
            ...
            let lib_program = lex_and_parse(&source, import_file.to_str().unwrap())
                .map_err(|e| format!("{}", e))?;
