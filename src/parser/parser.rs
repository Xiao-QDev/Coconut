...
impl Parser {
    ...
    fn parse_function_params(&mut self) -> Result<Vec<Parameter>, String> {
        ...
        loop {
            ...
            let name = self.consume_identifier()?;
            let param_type = if !self.check(Token::Comma) && !self.check(Token::RightParen) {
                self.parse_type()?
            } else {
                Type::Int
            };
    ...
    fn parse_type(&mut self) -> Result<Type, String> {
        ...
        match &token {
            ...
            _ => Err(self.error_at(line, &format!("expected type, got '{:?}'", token))),
        }
    }
    ...
    fn parse_statement(&mut self) -> Result<Statement, String> {
        ...
        if self.check(Token::MinusMinus) {
            if !self.is_valid_lvalue(&expr) {
                return Err(self.error("invalid decrement target"));
            }
            self.advance();
    ...
    fn parse_primary(&mut self) -> Result<Expr, String> {
        ...
        match &token {
            ...
            _ => Err(self.error_at(line, &format!("unexpected token '{:?}'", token))),
        }
    }
    ...
    fn error(&self, msg: &str) -> String {
        format!("line {}: {}", self.peek().line, msg)
    }

    fn error_at(&self, line: usize, msg: &str) -> String {
